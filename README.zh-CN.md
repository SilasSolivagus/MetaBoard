...
第一阶段——独立 dsh 插件的形态——已经实现，并且在真实会话上验证过；
[`docs/phase-1-acceptance.md`](docs/phase-1-acceptance.md) 是记录。当前分支在此之上
加了命令行、项目、留言流、会话绑定的认领，测试套件全程保持全绿。

还有两项验收没有走完，都因为需要一个真实的 dsh 会话才能跑：完整走一遍全流程，
以及两个会话并发认领同一个工作项。认领冲突这部分有单元测试覆盖，还没有在真实运行时上验过。
...
agent 正在做的工作项带着一个**认领**：第一个往它上面写的那个会话。认领还在的时候，别的
对话写它一律被拒——两个会话同时写同一个工作项就是这样被拦住的。`metaboard show <id>`
会印出认领在谁手里。任意 `metaboard status <id> <状态>` 都会解除认领，会话中途死掉时就靠
这一条把活儿收回来。
