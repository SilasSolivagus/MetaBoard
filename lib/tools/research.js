...
export function researchTool() {
  return defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{
        ...
        text: value.error === undefined
          ? `Recorded ${value.count} sources, ${value.unverified} without a source url. `
    ...
    async execute(args, exec) {
      try {
        // 准入:工作项要存在,而且要已立项。未立项的条目 agent 不许动 ——
        // 这条是 dashi 的规矩,在我们这儿由工具强制执行,不靠 agent 自觉。
        // 返回带 error 的结果而不是抛异常:抛了会跳过 presentationMeta,
        // 账本上留一行永久 running(R12/R16)。
        const state = workState(args.work)
        if (!state.ok) {
          return { query: args.query, count: 0, unverified: 0, sources: [], callId: exec.callId, error: state.reason }
        }
        // 认领:第一次在这个工作项上干活,把「等待认领」挪成「处理中」。
        claim(args.work, state.status)
