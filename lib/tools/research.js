  ...
  return defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{
        ...
        text: value.error === undefined
          ? `Found ${value.count} pieces on the same topic. (call id: ${value.callId} — pass this in a later draft's derivedFrom)`
    ...
    async execute(args, exec) {
      try {
        ...
        return { count: 3, sources: STUB_SOURCES, callId: exec.callId }
      } catch (error) {
        return { count: 0, callId: exec.callId, error: error instanceof Error ? error.message : String(error) }
      }
    },
  })
}
