...
import { createWork, storePath } from '../../store/works.mjs'
  ...
  return defineTool({
    ...
    async execute(args, exec) {
      try {
        ...
        // 发号与写下 create 在 createWork 的锁里一起完成。分开两步时两个会话会
        // 抢到同一个号,后一个的 create 被 fold 丢掉,而下面这行结果照样把
        // 「记为 t1:<自己的标题>」渲染进账本 —— 一句当场就假的话。
