  ...
  factory: (require) => {
    ...
    function shapeRows(snapshot) {
      ...
      rows.forEach(function (r) {
        ...
        //
        // R20:抑制还必须要求这次调用成功了。评审的可见行由 reviewDefinition
        // 从 user/message 产出,而那条消息是 execute 里 deferContext 写的——
        // 参数校验失败时 defineTool 在 execute 之前就抛了 ToolArgsError,
        // deferContext 从未执行,于是根本没有 user/message,没有评审行。
        // 此时这一行是这次失败在账本上唯一的痕迹,抑制掉它等于让整次失败消失。
        // 代价是评审的调用行在 running 期间会短暂可见——一次在飞的评审本来就
        // 是真事,它在完成时消失。
        if (d.referenceOnly && d.status === "done") return;
        ...
        out.push({
          ...
          // M2:解析到的目标不一定有信封 —— 上游被 run_code 子派发时
          // presentationMeta 不执行,那一行的 contentKind 是 undefined,而模型
          // 仍看得到 render 里的 callId 并照抄进 derivedFrom(四个工具描述里
          // 那句 "never inside run_code" 正是防这个的,说明它会发生)。
          // 退到 start 就已知的工具名,别渲染出字面量 undefined。
